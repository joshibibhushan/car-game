...
var parent = document.getElementById("opponentCar");
...
var a = setInterval(run_game,2000);
var b = setInterval(carCollision,20);
var c = setInterval(background,30);
...
function gameloop(){
	...
	this.createOpponent = function(){
		...
		document.getElementById("opponentCar").appendChild(that.element);
		that.x = getRandom(3);
		if(that.x<1)
			that.x = 50;
		...
		that.intervalId = setInterval(that.moveopponent, 100);
...
function carCollision(){
	//console.log("asdsd"+opponentList.length);
	for(var i=0; i<opponentList.length;i++){
		//console.log("asdsd");
		if(opponentList[i].element.style.left==mycar.style.left && opponentList[i].element.style.top.split('px')[0]>= 333){
			gameEnding.style.background="#199db3";
			gameEnding.style.padding="100px";
			gameEnding.style.zIndex="50";
